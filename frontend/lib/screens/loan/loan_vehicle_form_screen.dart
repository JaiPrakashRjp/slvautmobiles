...
/// Create / edit a LOAN vehicle. Same `vehicles` table as sale / rental
/// vehicles, scoped to module = loan (via [LoanVehicleService]) with a reduced
/// field set: Vehicle photo, Branch, Vehicle number, Model, Type, Insurance /
/// FC / Permit dates, and Insurance / FC / Permit (available flag + document).
/// Owned-hand and chassis are handled internally (owned-hand = second-hand).
class LoanVehicleFormScreen extends StatelessWidget {
  const LoanVehicleFormScreen({super.key, this.existing});
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      ...
      child: const _LoanVehicleFormView(),
    );
  }
}

class _LoanVehicleFormView extends StatefulWidget {
  const _LoanVehicleFormView();

  @override
  State<_LoanVehicleFormView> createState() => _LoanVehicleFormViewState();
}

class _LoanVehicleFormViewState extends State<_LoanVehicleFormView> {
  ...
  Future<void> _submit(CreateVehicleViewModel vm) async {
    ...
    if (vm.branch == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Required: Branch')),
      );
      return;
    }
    if (!formOk) return;
        ...
        child: Form(
          ...
          child: ListView(
            ...
            children: [
              ...
              DocUploadTile(
                label: 'Photo',
                fileName: vm.vehiclePhoto?.name,
                onTakePhoto: () => _takePhotoInto((d) => vm.vehiclePhoto = d),
                onUpload: () => _uploadFileInto((d) => vm.vehiclePhoto = d),
                onRemove: () => vm.vehiclePhoto = null,
              ),
              ...
              // ── Documents: Insurance / FC / Permit (available + document) ─
              Text('Documents',
                  style: AppTextStyles.label.copyWith(color: c.textSub)),
              const SizedBox(height: AppSpacing.sm),
              _paper(vm, 'Insurance', VehicleDocType.insurance, vm.insurance,
                  (v) => vm.insurance = v),
              _paper(vm, 'FC', VehicleDocType.fc, vm.fc, (v) => vm.fc = v),
  ...
  /// One document row: an "Available" checkbox + a document upload tile.
