"""Rental rent-collection reminder job — runs at 5 PM IST.

Separate from the 8 AM sale reminders (rent reads very differently and nudges
daily). Cron on the server:

    30 11 * * * cd /opt/slv-dev/backend && /opt/slv-dev/venv/bin/python -m app.jobs.run_rental_reminders >> /var/log/slv-rental-reminders.log 2>&1

(30 11 UTC = 17:00 IST.) Reminds renters with an unpaid rent period every day
