"""Rent payment proof — the screenshot attached when a rent payment is recorded.
...
class RentalPaymentDocument(Base):
    __tablename__ = "rental_payment_documents"

    id: Mapped[int] = mapped_column(BigInteger, Identity(always=True), primary_key=True)
    payment_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("rental_payments.id", ondelete="CASCADE"), nullable=False
    )
    ...
    payment: Mapped["RentalPayment"] = relationship(  # noqa: F821
